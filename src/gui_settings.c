...
void open_profile_settings_dialog(GtkButton *btn, gpointer user_data)
{
    ...
    gtk_window_set_default_size(GTK_WINDOW(dialog), 1400,950); // Wider window for 2-column layout

    // 2. Root Horizontal Layout Box (Left: Diagram | Right: Form Inputs)
    GtkWidget *main_hbox = gtk_box_new(GTK_ORIENTATION_HORIZONTAL, 16);
    set_margin(main_hbox, 16);

    // -------------------------------------------------------------
    // LEFT COLUMN: SVG Reference Diagram (Scalable GtkPicture)
    // -------------------------------------------------------------
    ...
    // Match labels with diagram keys (A, B, C, etc.)
